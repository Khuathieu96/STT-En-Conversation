...
async function getAvailableFreeModels(apiKey: string): Promise<string[]> {
  if (cachedFreeModels) {
    ...
    const ttl =
      cachedFreeModels.length > 0
        ? FREE_MODEL_CACHE_TTL_MS
        : EMPTY_FREE_MODEL_CACHE_TTL_MS;
    if (age < ttl) return cachedFreeModels;
  }
...
export async function callOpenRouter(apiKey: string, prompt: string): Promise<string> {
  ...
  for (const model of effectiveAttempts) {
    ...
    if (response.ok) {
      const data = (bodyJson ?? JSON.parse(bodyText)) as {
    ...
    const noEndpoint =
      response.status === 404 && modelErrorMessage.includes('No endpoints found');

    if (
      isBalanceExhaustedMessage(modelErrorMessage) ||
      isBalanceExhaustedMessage(bodyText)
    ) {
      throw new Error(
        'OpenRouter credits are exhausted. Please add funds or enable autopay.',
      );
    }
    ...
    lastError = new Error(
      `Model unavailable: ${model}. ${modelErrorMessage || 'No endpoint found.'}`,
    );
  }

  throw new Error(
    `No usable OpenRouter model found. Last error: ${lastError?.message ?? 'Unknown'}. ` +
    'Set OPENROUTER_MODEL to an available model from OpenRouter.',
  );
}
